...
const menu = [
  ...
  { menu: "Edit Album", link: `/partner/album/edit/${queryIdParam}` },
];
...
const videoListValue = ref(albumPartnerStore.album?.videos);
...
const description = ref(albumPartnerStore.album?.description ?? "");
...
fileNameThumbnail.value = albumPartnerStore.album?.name + "Thumbnail" ?? "";
fileNameCover.value = albumPartnerStore.album?.name + "Cover" ?? "";
...
const { errors, handleSubmit, defineField } = useForm({
  ...
  initialValues: {
    id: albumPartnerStore.album?.id,
    name: albumPartnerStore.album?.name ?? "",
    slug: albumPartnerStore.album?.slug ?? "",
    artist: albumPartnerStore.album?.artist.name,
    status: albumPartnerStore.album?.isActive,
    genre:
      albumPartnerStore.album?.styles
...
artist.value = albumPartnerStore.album?.artist.name;
artistListId.value = albumPartnerStore.album?.artist.id;
genreListId.value = albumPartnerStore.album?.styles.map((item) => item.id);
videoListId.value = albumPartnerStore.album?.videos?.map((item) => item.id);
...
const onSubmit = handleSubmit(async (value) => {
  ...
  if (
    ...
    await albumPartnerStore.updateAlbum(formData);
    if (albumPartnerStore.error) {
      ...
      navigateTo("/partner/album/detail/" + value.id);
      isLoading.value = false;
    }
  }
});
...
const deleteAlbum = async (index: string) => {
  isLoading.value = true;
  await albumPartnerStore.deleteAlbumById(index);
  if (albumPartnerStore.error) {
    const errMsg = albumPartnerStore.error.message;
    msgError.value = errMsg;
    showSweetAlert(errMsg, "error", 4000, true, "top-end");
    ...
    showSweetAlert(
      "สำเร็จ! ลบข้อมูลอัลบั้มเรียบร้อยแล้ว",
      "success",
      4000,
      true,
      "top-end",
...
<template>
  <div class="edit-album-partner-page">
    <div class="wrapper-content">
      ...
      <div class="section-form">
        <div class="d-flex align-items-center justify-content-between">
          <p class="title-album">แก้ไขอัลบัม</p>
          <Button
            data-bs-toggle="modal"
            class="btn btn-delete-member"
            data-bs-target="#deleteModal"
            ><i class="las la-trash-alt icon-trash"></i> ลบอัลบั้ม</Button
          >
        </div>
        ...
        <ConfirmModal
          :confirm-modal="deleteAlbum"
          :id-member="queryIdParam"
          :id-modal="'deleteModal'"
          :title="'ยืนยันการลบ'"
          :message="`ยืนยันการลบอัลบั้ม ‘${albumPartnerStore.album?.name}’ เมื่อยืนยันแล้วจะไม่สามารถยกเลิกได้`"
        >
        </ConfirmModal>
      </div>
    </div>
...
<style
  src="assets/scss/pages/partner/album/edit-album-partner.scss"
  lang="scss"
  scoped
></style>
